// config_emmc.cpp : Defines the exported functions for the DLL application.
//
			...
			void *param)
{
	...
	struct _st_icinfo {
		...
		(unsigned int)chip->partition_info[0].read_parameter,
                (unsigned int)chip->partition_info[0].timing,
	};

        assert(chip->partition_info[0].size_in_bytes % chip->partition_info[0].page_size_in_byte == 0);
